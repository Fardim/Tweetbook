    ...
    public class IdentityService: IIdentityService
    {
        ...
        public async Task<AuthenticationResult> LoginAsync(string email, string password)
        {
            ...
            if (!userHasValidPassword)
            {
                return new AuthenticationResult
                {
                    Errors = new[] { "User/Password combination does not match" }
                };
            }
        ...
        public async Task<AuthenticationResult> RefreshTokenAsync(string token, string refreshToken)
        {
            ...
            {
                return new AuthenticationResult { Errors = new[] { "This token hasnot expired yet." } };
            }
            var jti = validatedToken.Claims.Single(x => x.Type == JwtRegisteredClaimNames.Jti).Value;
            var storedRefreshToken = await _context.RefreshTokens.SingleOrDefaultAsync(d => d.Token == refreshToken);
            ...
            {
                return new AuthenticationResult { Errors = new[] { "This refresh token is used." } };
            }
            ...
            var user = await _userManager.FindByIdAsync(validatedToken.Claims.Single(d => d.Type == "id").Value);
            return await GenerateAuthenticationResultForUserAsync(user);
        }
